...
const RUNNER_LINES: string[] = [
  ...
  " * package.json for the Baileys version. Point BA PANEL_URL at the panel to",
  " * stream the console back, or run it standalone and tail stdout.",
  " */",
  "",
  ...
  "  if (!PANEL_URL) return;",
  "  try {",
  "    await fetch(`${PANEL_URL}/api/wings/log`, {",
  "      method: 'POST',",
  "      headers: { 'content-type': 'application/json',",
  "        authorization: `Bearer ${process.env.PANEL_KEY ?? ''}` }},",
  "      body: JSON.stringify({ session: SESSION_ID, level, message }),",
  "    });",
  "  } catch (err) { console.error('log forward failed', err.message); }",
  "}",
  "",
